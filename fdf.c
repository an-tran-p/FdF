...
/*   Updated: 2025/04/13 17:38:05 by atran            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	handle_input(mlx_key_data_t keydata, void *param)
{
	...
	{
		if ((window->org_h + window->org_w - window->min_v) * (window->zoom + 1)
			* 0.5 < HEIGHT / 2 && window->org_w * (window->zoom + 1) < WIDTH / 2
			&& window->max_v * (window->zoom + 1) < HEIGHT / 2)
			window->zoom++;
	}
